...
export default function CropActivityFolder({ auth }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (editingFolderId) {
            ...
            router.patch(`/admin/cropactivity/update/${editingFolderId}`, payload, {
                onSuccess: (response) => {
    ...
    return (_jsxs(AdminLayout, { user: auth.user, header: _jsx("h2", { className: "text-xl mt-2 text-gray-800 leading-tight", children: "Crop Activity Management" }), children: [_jsx(Head, { title: "Management" }), _jsx(ToastContainer, {}), _jsxs("div", { className: "flex justify-between mb-3", children: [_jsx(Search, { onSearch: handleSearch }), _jsx(PrimaryButton, { className: "text-sm justify-center align-content-center rounded-lg text-white", onClick: () => openModal(), children: _jsx("span", { className: "flex gap-2", children: "Primary Button" }) })] }), _jsxs("span", { className: "mt-5 text-sm text-slate-400", children: ["Total: ", filteredFolders.length] }), _jsx("div", { className: "flex flex-wrap gap-5 mt-2", children: filteredFolders.map((folder) => folder ? (_jsxs("div", { className: "flex justify-between border border-slate-300 w-[300px] h-[45px] rounded-2xl p-3 cursor-pointer", onClick: () => handleView(folder.id), children: [_jsx("div", { className: "border-b border-slate-300 mb-2", children: _jsx("p", { className: "text-sm font-semibold text-slate-600", children: folder.title }) }), _jsx("div", { className: "text-xs text-slate-500 mb-2", children: _jsx("span", { children: folder.date
                                    ...
                                    : "No date provided" }) }), _jsxs("div", { className: "b-0 flex gap-3", children: [_jsx("button", { className: "text-xs text-red-500 mt-auto", onClick: (e) => {
                                        ...
                                        e.stopPropagation();
                                        openModal(folder);
                                    }, children: _jsx(PencilIcon, { size: 20 }) })] })] })) : null) }), _jsx(Modal, { show: isModalOpen, maxWidth: "lg", closeable: true, onClose: closeModal, children: _jsxs("div", { className: "p-6 text-center", children: [_jsx("h2", { className: "text-xl font-semibold mb-4", children: editingFolderId
                                ? "Edit Crop Activity"
