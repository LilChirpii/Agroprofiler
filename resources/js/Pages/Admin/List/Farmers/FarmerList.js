...
export default function FarmerList({ auth, farmers, barangays = [], }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setFarmersData((prevFarmers) => prevFarmers.filter((farmer) => farmer !== formattedFarmer));
            if (axios.isAxiosError(error) && error.response) {
    ...
    return (_jsxs(AdminLayout, { user: auth.user, header: _jsx(_Fragment, { children: _jsx("div", { className: "flex w-full justify-between", children: _jsx("h2", { className: "text-[24px]  mt-2 font-medium text-green-800 leading-tight", children: "Farmers Management" }) }) }), children: [_jsx(Head, { title: "Farmers Management" }), _jsx(ToastContainer, {}), _jsxs("div", { className: "flex w-full gap-4", children: [_jsxs("div", { className: "flex gap-2", children: [_jsx(TextInput, { type: "file", onChange: handleFileChange, accept: ".xlsx,.csv", className: "rounded-md border p-1" }), _jsxs(PrimaryButton, { className: "flex gap-2 rounded-md", onClick: handleImport, children: [_jsx(Import, { size: 16 }), "Import"] })] }), _jsx(PrimaryButton, { className: "text-sm justify-center align-content-center rounded-lg text-white", onClick: openModal, children: _jsxs("span", { className: "flex gap-2", children: [_jsx(PlusIcon, { size: 18 }), "Add new"] }) }), _jsxs(SecondaryButton, { onClick: handleMultipleDelete, disabled: selectedIds.length === 0 || loading, style: {
