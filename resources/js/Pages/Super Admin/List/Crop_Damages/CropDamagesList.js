...
import { Autocomplete, Box, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, TextField, } from "@mui/material";
...
const CropDamages = ({ auth }) => {
    ...
    const fetchFarms = async (farmerId) => {
        ...
        setLoadingFarms(true);
        try {
            const response = await axios.get(`/search/data/farms?farmer_id=${farmerId}`);
            setFarms(response.data.map((farm) => ({
                value: farm.value,
            ...
            if (response.data.message) {
                ...
                setCropDamages((prevData) => {
                    if (formData.id) {
                        ...
                        return prevData.map((item) => item.id === formData.id
    ...
    return (_jsxs(Authenticated, { user: auth.user, header: _jsx(_Fragment, { children: _jsx("div", { className: "flex w-full justify-between", children: _jsx("h2", { className: "text-[24px] mt-2 font-semibold text-green-600 leading-tight", children: "Crop Damages Management" }) }) }), children: [_jsx(Head, { title: "Damages List" }), _jsxs("div", { className: "flex gap-4", children: [_jsxs("div", { className: "flex gap-2", children: [_jsx(TextInput, { type: "file", accept: ".csv", onChange: handleImportFileChange, className: "rounded-md border p-1" }), _jsxs(PrimaryButton, { onClick: handleUpload, className: "flex gap-2", children: [_jsx(Import, { size: 20 }), " Import"] })] }), _jsxs(PrimaryButton, { onClick: () => setOpenDialog(true), children: [_jsx(Plus, { size: 24 }), "Add Data"] }), _jsxs(SecondaryButton, { onClick: handleMultipleDelete, disabled: selectedIds.length === 0 || loading, style: {
                            ...
                            height: "520px",
                        }, children: _jsx(DataGrid, { rows: cropDamages, columns: columns, getRowId: (row) => row.id, initialState: {
                                pagination: {
                                ...
                                border: "none",
                                        ...
                                        }, filterOptions: (options) => options, renderOption: (props, option) => (_createElement("li", { ...props, key: option.value }, option.label)), renderInput: (params) => (_jsx(TextField, { ...params, label: "Select Farmer", variant: "outlined", fullWidth: true })) }), _jsx("br", {}), _jsx("br", {}), _jsx(Autocomplete, { options: farms, getOptionLabel: (option) => option.label, loading: loadingFarms, onChange: (event, newValue) => handleSelectChange(newValue, "farm_id"), filterOptions: (options, state) => options.filter((option) => option.label
                                            .toLowerCase()
                                            .includes(state.inputValue.toLowerCase())), renderOption: (props, option) => (_createElement("li", { ...props, key: option.value }, option.label)), renderInput: (params) => (_jsx(TextField, { ...params, label: "Select Farm", variant: "outlined", fullWidth: true, InputProps: {
                                                ...params.InputProps,
