...
const Commodities = ({ auth }: PageProps) => {
    ...
    const handleSave = async () => {
        ...
        try {
            let response;
            // Adjust the request method for update operations
            if (isEdit) {
                response = await axios.put(
                    `/commodities/update/${formData.id}`,
                    formData // Directly sending formData, no need to add _method field here
                );
            } else {
            ...
            if (response.data.errors) {
                ...
                // Refresh commodity list after successful save
                fetchCommodities();
                handleClose();
            }
        } catch (error) {
