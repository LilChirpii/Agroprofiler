...
const Commodities = ({auth}: PageProps) => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('/api/commodity-categories');
  ...
  const handleSave = async () => {
    try {
      if (isEdit) {
        await axios.put(`/api/commodity-categories/${formData.id}`, formData);
      } else {
        await axios.post('/api/commodity-categories', formData);
      }
      fetchData();
      handleClose();
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    try {
      await axios.delete(`/api/commodity-categories/${id}`);
      fetchData();
    } catch (error) {
