...
const Commodities = ({ auth }: PageProps) => {
    ...
    const handleSave = async () => {
        try {
           if (isEdit) { console.log(`PUT request to: /commodities-categories/update/${formData.id}`); await axios.put(`/commodities-categories/update/${formData.id}`, formData); handleClose(); } else { const response = await axios.post("/commodity-categories/store", formData); fetchData(); toast.success(response.data.message, { draggable: true, closeOnClick: true, }); handleClose(); }
            } else {
