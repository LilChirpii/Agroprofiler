...
export default function FarmProfile({ auth, farmer }: FarmersListProps) {
    ...
    const handleUpdateCropActivity = async () => {
        ...
        try {
            ...
            if (formData.img) {
                dataToSend.append("file_path", formData.img); // Align with Laravel's file validation
            }
    ...
    const allocationData = allocations.map((allocation) => ({
        id: allocation.id,
        allocation: allocation.allocation_type.name,
        dateReceived: new Date(allocation.date_received).toLocaleDateString(),
        onEdit: handleUpdateAllocation,
        onDelete: deleteData,
    }));
                ...
                <div className="relative col-span-3 right-0 h-full px-6 py-4">
                    ...
                    <div className="w-full h-full mt-4">
                        <Tabs>
                            ...
                            <Tab label="Crop Activities">dasd</Tab>
                        </Tabs>
                    </div>
                </div>
