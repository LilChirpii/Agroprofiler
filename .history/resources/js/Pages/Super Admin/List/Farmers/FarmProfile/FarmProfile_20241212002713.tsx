...
export default function FarmProfile({ auth, farmer }: FarmersListProps) {
    ...
    const createOrUpdateData = async () => {
        ...
        // Client-side validation for required fields based on modal type
        ...
        const missingFields = requiredFields.filter(
            (field) =>
                !formData[modalType] ||
                formData[modalType][field] === "" ||
                formData[modalType][field] === 0
        );
        ...
        try {
            const method = formData[modalType].id ? "put" : "post";
            const dataToSend = new FormData();

            // Append farmer_id (common to all forms)
            dataToSend.append("farmer_id", farmer.id.toString());

            // Iterate over the fields of the specific form (based on modalType)
            for (const [key, value] of Object.entries(formData[modalType])) {
                if (key === "proof" && value instanceof File) {
                    // Handle file input for proof
                    dataToSend.append(key, value);
                } else if (value !== null && value !== undefined) {
                    // Append other values as string (FormData does not accept other types)
            ...
            // Determine the appropriate endpoint based on modal type
            let endpoint = "";
            if (modalType === "allocation") {
                endpoint =
                    method === "post"
                        ? "/allocations/store"
                        : `/allocations/update/${formData[modalType].id}`;
            } else if (modalType === "damage") {
                endpoint =
                    method === "post"
                        ? "/store/cropdamages"
                        : `/cropdamages/update/${formData[modalType].id}`;
            } else if (modalType === "farm") {
                endpoint =
                    method === "post"
                        ? "/farms/store"
                        : `/farms/update/${formData[modalType].id}`;
            }
            ...
            toast.success(
                `Successfully ${
                    formData[modalType].id ? "updated" : "created"
                } the ${modalType} data!`
            );
