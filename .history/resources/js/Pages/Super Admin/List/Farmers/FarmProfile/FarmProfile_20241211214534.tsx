...
export default function FarmProfile({ auth, farmer }: FarmersListProps) {
    ...
    console.log(farmer);
    console.log("farmer crop damagaes", farmer.crop_damages);
    console.log("farmer allocations: ", farmer.allocations);
    console.log("farmer farms: ", farmer.farms);
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            console.log("Crop Damages data", cropDamages);
            console.log("Allocations data", farms);
        } catch (error) {
    ...
    const createOrUpdateData = async () => {
        ...
        try {
            ...
            dataToSend.append("farmer_id", farmer.id);
            dataToSend.append("brgy_id", farmer.barangay.id);
            for (const [key, value] of Object.entries(formData)) {
                if (key === "proof" && value instanceof File) {
                    dataToSend.append(key, value); // Attach the file
                } else if (value !== null && value !== undefined) {
                    dataToSend.append(key, value.toString()); // Convert other values to strings
                }
            }
    ...
    const damageColumns: GridColDef[] = [
        { field: "id", headerName: "#", width: 90 },
        { field: "crop_damage_cause_id", headerName: "Cause", width: 250 },
        {
