...
export default function FarmProfile({ auth, farmer }: FarmersListProps) {
    ...
    const [formData, setFormData] = useState<any>({
        farmer: {
            id: "", // Default value to avoid undefined
            farmer_id: farmer?.id, // Static value from `farmer` prop or state
            brgy_id: farmer?.barangay?.id, // Static value from `farmer.barangay`
            ...formData.farmer, // Any dynamic data passed through formData (e.g., when editing)
        },
        barangay: {
            id: "", // Default value for barangay
            ...formData.barangay, // Any dynamic data passed through formData (e.g., when editing)
        },
        // Other fields
    });
    ...
    const brgyId = farmer.barangay.id;
                        ...
                        <div className="p-10 h-50">
                            ...
                            >
                                <input
                                    type="text"
                                    name="brgy_id"
                                    value={formData.farmer?.barangay?.id || ""} // Ensure brgy_id is read-only and constant
                                    readOnly
                                    onChange={(e) =>
                                        setFormData({
                                            ...formData,
                                            farmer: {
                                                ...formData.farmer,
                                                brgy_id: e.target.value, // Not needed since brgy_id is constant
                                            },
                                        })
                                    }
                                />

                                {/* farmer_id input (also constant) */}
                                <input
                                    type="text"
                                    name="farmer_id"
                                    value={formData.farmer?.id || ""} // Make sure it's not undefined or null
                                    readOnly // Prevent user from editing
                                    onChange={(e) => {
                                        // You can keep the onChange handler to avoid any warnings, but it's not needed for a read-only field
                                        setFormData({
                                            ...formData,
                                            farmer: {
                                                ...formData.farmer,
                                                id: e.target.value, // This will not be called since it's read-only
                                            },
                                        });
                                    }}
                                />
