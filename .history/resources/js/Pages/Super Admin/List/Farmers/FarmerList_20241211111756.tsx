...
export default function FarmerList({
    ...
    const handleEdit = (farmerId: number) => {
        ...
        if (farmer) {
            setSelectedFarmer(farmer); // Set the full farmer data
    ...
    const handleDelete = async (data: CropDamage) => {
        console.log("Deleting crop damage with ID:", data.id); // Ensure this is logged correctly

        if (!data.id) {
            toast.error("Invalid crop damage ID!", {
                draggable: true,
                closeOnClick: true,
            });
            return; // Prevent the delete if ID is invalid
        }

        if (
            window.confirm("Are you sure you want to delete this crop damage?")
        ) {
            try {
                // Make the delete request using POST method with _method spoofing
                await axios.post(`/cropdamages/destroy/${data.id}`, null, {
                    headers: {
                        "X-CSRF-TOKEN":
                            document
                    ...
                    params: {
                        // Spoof the DELETE method with _method parameter
                        _method: "DELETE",
                    },
                });
                ...
                setCropDamageCause((prevData) =>
                    prevData.filter(
                        (cropDamageCause) => cropDamageCause.id !== data.id
                    )
                );
