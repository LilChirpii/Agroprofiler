...
export default function AllocationList({
    ...
    const columns: GridColDef[] = [
        { field: "id", headerName: "#", width: 50 },
        {
            ...
            headerName: "Type",
            width: 100,
            valueGetter: (value, row) => row.allocation.allocation_type,
        },
        {
            field: "farmer_id",
            headerName: "Receiver",
    ...
    const handleInputChange = (
        ...
        setSelectedAllocation((prevAllocation) => {
            ...
            // Handling different input types (select, date, etc.)
            let updatedValue = value;

            // For the date input, we need to parse the date correctly
            if (name === "date_received" && value) {
                updatedValue = new Date(value); // Convert string to Date object
            }

            // For select options, handle it as a string (ID values)
            if (name === "brgy_id" || name === "commodity_id") {
                updatedValue = value ? parseInt(value) : null; // Ensure it's parsed as a number (or null)
            }
    ...
    const handleUpdateSelectChange = (selectedOption: any, name: string) => {
        setSelectedAllocation((prevAllocation) => {
            ...
            return {
                ...prevAllocation,
                [name]: selectedOption ? selectedOption.value : null, // Safely assign
            };
        });
    };
