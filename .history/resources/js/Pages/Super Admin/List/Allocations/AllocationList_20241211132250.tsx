...
export default function AllocationList({
    ...
    const handleEdit = (allocations: Allocation) => {
        setSelectedAllocation(allocations);
    ...
    const handleUpdateSelectChange = (
        ...
        setSelectedAllocation((prev) => ({
            ...prev,
            // Ensure that the field is always assigned a valid value (fallback to a default if needed)
            [field]: selectedOption ? selectedOption.value : prev?.[field] || 0, // Fallback to previous value if not provided
        }));
    };
    ...
    const [selectedAllocation, setSelectedAllocation] =
        useState<Allocation | null>({
            id: 0, // Default id
            allocation_type_id: "", // Default allocation_type_id
            allocation_type: "", // Default allocation_type
            farmer: {
                id: 0,
                firstname: "",
                lastname: "",
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await axios.put(
                `/allocations/update/${selectedAllocation}`,
                updates,
                {
                    headers: {
