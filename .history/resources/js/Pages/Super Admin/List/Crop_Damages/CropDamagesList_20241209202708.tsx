...
export default function CropDamagesList({
    ...
    const columns: GridColDef[] = [
        ...
        },
        {
            ...
            headerName: "Partially Damaged Area",
            width: 100,
        },
    ...
    const handleEdit = (cropDamageId: number) => {
        if (!cropDamages) {
            console.error("User data is not available");
            return;
        }

        const cropDamage = cropDamages.find(
            (cropDamage) => cropDamage.id === cropDamageId
        );

        if (cropDamage) {
            setSelectedDamage(cropDamage); // Set the full cropDamage data
            setIsUpdateModalOpen(true); // Open the modal
            console.log("Selected cropDamage: ", cropDamage); // Log the selected user data
        } else {
