...
export default function CropDamagesList({
    ...
    const [cropDamages, setCropDamages] = useState<DamageProps[]>();
    const [loading, setLoading] = useState(false);

    const fetchCropDamagesData = () => {
        setLoading(true);

        axios
            .get("/cropdamages/data")

            .then((response) => {
                const data = response.data;
                setCropDamages(data);
                console.log("Damages data: ", cropDamages);
            })
            .catch((error) => {
    ...
    const rows = damageData.map((damage: Damage) => ({
        ...
        cause: damage.cropDamageCause,
        remarks: damage.remarks,
    }));

    const columns: GridColDef[] = [
        { field: "id", headerName: "#", width: 50 },
        { field: "farmer_id", headerName: "Name", width: 120,  valueGetter: (value, row) => {
        ...
        { field: "commodity_id", headerName: "Commodity", width: 70,  valueGetter: (value, row) => {
            return row?.commodity?.name || "Not Assigned";
        }, },
        {
            field:  "cropDamageCause",
            headerName:  "Cause",
            width: 50,
            valueGetter: (value, row) => {
                return row?.crop_damage_cause?.name || "Not Assigned";
            },

        },
        { field: "total_damaged_area", headerName: "Total Damaged Area", width: 100 },
        { field: "partially_damaged_area", headerName: "Partially Damaged Area", width: 100 },
        { field: "area_affected", headerName: "Area Affected", width: 100 },
    ...
    const handleEdit = (cropDamages: Damage) => {
        setSelectedDamage(cropDamages);
        setIsUpdateModalOpen(true);
        console.log(selectedDamage);
    };

    const handleDelete = async (cropDamages: Damage) => {
        if (
            window.confirm(
            ...
            try {
                await router.delete(`/cropdamages/destroy/${cropDamages}`);
