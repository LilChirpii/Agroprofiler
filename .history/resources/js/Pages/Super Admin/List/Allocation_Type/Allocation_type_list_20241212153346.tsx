...
export default function AllocationTypeList({ auth }: PageProps) {
    ...
    const handleSelectChange = (
        event: SelectChangeEvent<string[]>, // Change to SelectChangeEvent for string[] type
        key: string
    ) => {
        const value = event.target.value;
        setFormData((prevData) => ({
            ...prevData,
            [key]: value, // No need for the typeof check
        }));
    };
    ...
    const rows: AllocationTypeRow[] = (allocationTypes || []).map(
        (type, index) => ({
            ...
            // Map commodities to an array of strings
            ...
            // Map barangays to an array of strings
            ...
            // Map crop damage causes to an array of strings
            ...
            // Map eligibilities to an array of strings
    ...
    //             commodityIds: allocation.allocation_type_commodities.map(
    //                 (c: { id: number }) => c.id
    //             ),
    //             barangayIds: allocation.allocation_type_barangays.map(
    //                 (b: { id: number }) => b.id
    //             ),
    //             cropDamageCauseIds:
    ...
    //                     (d: { id: number }) => d.id
    //                 ),
    //             eligibilityIds: allocation.allocation_type_elligibilities.map(
    //                 (e: { id: number }) => e.id
    //             ),
    //         });
    //         setModalOpen(true);
