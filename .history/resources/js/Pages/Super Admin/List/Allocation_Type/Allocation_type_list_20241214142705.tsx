...
export default function AllocationTypeList({ auth }: PageProps) {
    ...
    const handleCheckboxChange = (
        ...
        setFormData((prevState) => {
            const selectedIds = [...(prevState[field] as number[])];
            if (checked) {
                selectedIds.push(id);
            } else {
            ...
            setFormData({
                ...
                commodityIds:
                    allocation.allocation_type_commodities?.map(
                        (c) => c.commodity_id // Change c.id to c.commodity_id
                    ) || [],
                barangayIds:
                    allocation.allocation_type_barangays?.map(
                        (b) => b.barangay_id // Similarly, ensure you're using barangay_id
                    ) || [],
                cropDamageCauseIds:
                    allocation.allocation_type_crop_damage_causes?.map(
                        (d) => d.crop_damage_cause_id // Similarly for crop damage causes
                    ) || [],
                eligibilityIds:
                    allocation.allocation_type_elligibilities?.map(
                        (e) => e.elligibility_id // Similarly for eligibilities
                    ) || [],
            });
    ...
    const handleUpdateSubmit = async () => {
        try {
            const response = await axios.put(
                `/api/allocation-types/${formData.id}`,
                formData
            );
                                ...
                                >
                                    ...
                                    {commodities.map((commodity) => {
                                        const isChecked =
                                            ...
                                            ); // Change to commodity.commodity_id
                                        console.log(
                                            "Commodity ID:",
                                            commodity.commodity_id,
                                            "IsChecked:",
