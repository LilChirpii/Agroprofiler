...
export default function AllocationTypeList({ auth }: PageProps) {
    ...
    return (
        ...
        onChange={(e) => handleSelectChange(e, 'barangayIds')}
        renderValue={(selected) =>
          (selected as number[])
            .map(
              (id) =>
                barangays.find((item) => item.id === id)?.name
            )
            .join(', ')
        }
      >
        <MenuItem value={0} onClick={() => handleSelectChange({ target: { value: [0] } }, 'barangayIds')}>
          <Checkbox checked={isNoSelected} />
          <ListItemText primary="NO" />
        </MenuItem>
        ...
        {barangays.map((barangay) => (
          <MenuItem
            key={barangay.id}
            value={barangay.id}
            disabled={isNoSelected} // Disable other items if "NO" is selected
          >
            <Checkbox
              checked={formData.barangayIds.includes(barangay.id)}
              disabled={isNoSelected} // Disable checkbox if "NO" is selected
            />
