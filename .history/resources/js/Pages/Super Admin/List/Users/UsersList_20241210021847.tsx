...
const UsersList = ({ auth }: UserProps) => {
    ...
    const handleUpdate: FormEventHandler = async (e) => {
        ...
        // If there is a new profile picture, append it to the form data
        if (selectedUser?.pfpFile) {
            formData.append("profile_picture", selectedUser.pfpFile);
        }

        try {
            await axios.post(`/users/update/${selectedUser?.id}`, formData, {
                headers: {
                    "X-CSRF-TOKEN":
                        document
