...
const UsersList = ({ auth, users }: UserProps) => {
    ...
    const handleEdit = (userData: UserProps) => {
        // Assuming 'users' is the array of user objects available in your component
        const foundUser: User | undefined = users.find(
            (user: User) => user.id === userData.id
        );

        if (foundUser) {
            setSelectedUser(foundUser); // Assuming setSelectedUser is your state setter for editing
        } else {
    ...
    //     try {
    //         const response = axios.get(`users/${userData}`);
