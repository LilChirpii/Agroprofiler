                    ...
                    headers: {
                        "X-CSRF-TOKEN":
                            document
                                ...
                                ?.getAttribute("content") || "", // Default to an empty string if null
                    },
                }
            );

            console.log(`/users/update/${selectedUser?.id}`);

            // Refetch and update the user list
            fetchUserData();
            setUserData((prevData) =>
                selectedUser
                    ? prevData
                          .filter((userData) => userData.id !== selectedUser.id)
                          .concat(selectedUser) // Add the updated user if it's not null
                    : prevData
            );
