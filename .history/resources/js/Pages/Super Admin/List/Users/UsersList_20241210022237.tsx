...
const UsersList = ({ auth }: UserProps) => {
    ...
    const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (files && files.length > 0) {
            ...
            setSelectedUser((prevUser) => {
                if (!prevUser) return null; // Ensure `prevUser` exists
                return {
                    ...prevUser,
    ...
    const handleUpdate: FormEventHandler = async (e) => {
        ...
        // Convert the profile picture to a base64 string if available
        let profilePictureBase64 = "";
        if (selectedUser?.pfp instanceof File) {
            const reader = new FileReader();
            reader.onloadend = async () => {
                profilePictureBase64 = reader.result as string; // base64 string

                // Now, send the request with base64 encoded image and other fields
                await sendUpdateRequest(profilePictureBase64);
            };
            reader.readAsDataURL(selectedUser.pfp); // Convert file to base64
        } else {
            await sendUpdateRequest(profilePictureBase64); // If no file, send empty string or old pfp
        }
    };

    const sendUpdateRequest = async (profilePictureBase64: string) => {
        try {
            await axios.post(
                `/users/update/${selectedUser?.id}`, // Use POST, but spoof it as PUT
                {
                    ...
                    pfp: profilePictureBase64, // Send the base64 string of the profile picture
                },
                {
                    headers: {
                        "X-CSRF-TOKEN":
