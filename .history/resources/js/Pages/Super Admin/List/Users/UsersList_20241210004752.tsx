...
const UsersList = ({ auth }: UserProps) => {
    ...
    const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (files && files.length > 0) {
            ...
            setSelectedUser((prevUser) => ({
                ...prevUser,
                pfp: previewUrl, // Set the preview URL for display
            }));

            // Optionally, save the file for uploading
            setNewUser("pfp", file);
        } else {
            // Reset the preview and file
            setSelectedUser((prevUser) => ({
                ...prevUser,
                pfp: null,
            }));
    ...
    const handleUpdate: FormEventHandler = async (e) => {
        ...
        try {
            ...
            // Type narrowing for 'pfp'
            if (selectedUser.pfp instanceof File) {
                // If pfp is a File, append it directly
                formData.append("pfp", selectedUser.pfp);
            } else if (typeof selectedUser.pfp === "string") {
                // If pfp is a string, append it as a string
                formData.append("pfp", selectedUser.pfp);
            } else {
                // Handle the case where pfp is null or undefined
                console.error("Profile picture is not available or invalid");
            }

            await axios.patch(`/users/update/${selectedUser.id}`, formData);
