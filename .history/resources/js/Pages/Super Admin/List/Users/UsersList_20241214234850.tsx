...
const UsersList = ({ auth }: UserProps) => {
    ...
    const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (files && files.length > 0) {
            ...
            // Optionally, you can also store the file data or update the selectedUser state
            setSelectedUser((prevUser) => {
                if (!prevUser) return null; // Ensure `prevUser` exists
                return {
                    ...prevUser,
                    pfp: file, // You may store the actual file for uploading later
                };
            });
        } else {
    ...
    const handleSubmit: FormEventHandler = async (e) => {
        ...
        // Append each field in `newUser` to FormData
