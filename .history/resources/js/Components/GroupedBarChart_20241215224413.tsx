    ...
    const handleOpenModal = () => {
        const tableData = chartData.map((row) => {
            ...
            if (
                ...
                data[row.name].commodities
            ) {
                console.log("Processing commodity categories for:", row.name);
                console.log("Category data:", data[row.name].commodities);

                data[row.name].commodities.forEach((category) => {
                    console.log(
                        "Category name:",
                        category.commodities_category_name
                    );
                    if (
                        !selectedCategory ||
                        category.commodities_category_name === selectedCategory
                    ) {
                        console.log("Selected category matches or is not set.");
                        category.commodities.forEach((commodity) => {
                            ...
                            rowDetails[commodity.name] = commodity.count || 0;
                            rowDetails.total += commodity.count || 0;
                        });
                    }
                });
            }
