...
const Heatmap: React.FC<HeatmapProps> = ({ distributionData }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [geoDataResponse, commodityResponse] = await Promise.all([
                    axios.get("/Digos_City.geojson"),
                    axios.get("/commodity-categories-show"), // Adjust this endpoint based on your routes
                ]);

                setGeoData(geoDataResponse.data);
                setCommodityCategories(commodityResponse.data);

                if (mapRef.current && geoDataResponse.data) {
                    geoJsonLayer.current = new LeafletGeoJSON(
    ...
    const getOptionsForSubtype = () => {
        ...
        // Default subtypes for fixed views
        switch (view) {
            case "allocations":
                return [
                    { value: "All", label: "All" },
            ...
            case "highValue":
                return [
                    { value: "All", label: "All" },
                    { value: "Fruit-Bearing", label: "Fruit-Bearing" },
