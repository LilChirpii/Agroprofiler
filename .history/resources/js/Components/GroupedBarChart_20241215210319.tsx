...
const GroupedBarChart: React.FC<GroupedBarChartProps> = ({
    ...
    const chartData = Object.keys(data).map((barangay) => {
        ...
        if (
            ...
            Object.keys(entry.commodities_categories).forEach((category) => {
                const categoryData = entry.commodities_categories[category];
                Object.keys(categoryData).forEach((subcategory) => {
                    const key = `${category} - ${subcategory}`;
                    rowData[key] = categoryData[subcategory] || 0; // Add the count for individual commodities under the category
                });
            });
        }
    ...
    const handleOpenModal = () => {
        const tableData = chartData.map((row) => {
            ...
            if (
                distributionType === "commodityCategories" &&
                data[row.name].commodities_categories
            ) {
                Object.keys(data[row.name].commodities_categories).forEach(
                    (category) => {
                        const categoryData =
                            data[row.name].commodities_categories[category];
                        Object.keys(categoryData).forEach((subcategory) => {
                            rowDetails[`${category} - ${subcategory}`] =
                                categoryData[subcategory] || 0;
                            rowDetails.total += categoryData[subcategory] || 0;
                        });
                    }
                );
            }
        ...
        console.log(`Distribution Type Selected: ${distributionType}`);
        console.log("Selected Data for Modal:", tableData);
