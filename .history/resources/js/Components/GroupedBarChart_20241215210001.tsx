        ...
        if (distributionType === "commodityCategories" && entry.commodities) {
            entry.commodities.forEach((categoryData) => {
                categoryData.commodities.forEach((commodity) => {
                    const key = `${categoryData.commodities_category_name} - ${commodity.name}`;
                    rowData[key] = commodity.count || 0; // Add the count for individual commodities under the category
                });
            });
        }
    ...
    const handleOpenModal = () => {
        const tableData = chartData.map((row) => {
            ...
            if (
                ...
                Object.keys(entry.commodities_categories).forEach(
                    (category) => {
