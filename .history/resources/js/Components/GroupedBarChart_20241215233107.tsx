...
const GroupedBarChart: React.FC<GroupedBarChartProps> = ({
    ...
    const chartData = Object.keys(data || {}).map((barangay) => {
        ...
        if (
            ...
            const categoryData = row.commodities_categories[categoryName] || {}; // Handle missing categoryData gracefully

            // Loop over the subcategories (commodities) for the given category
            Object.keys(categoryData).forEach((subcategory) => {
                const count = categoryData[subcategory] || 0; // Make sure we get the count
                rowDetails[subcategory] = count; // Assign the count to the subcategory
                subcategories.push(subcategory); // Track the subcategories
                rowDetails.total += count; // Add to the total
            });
        }
    ...
    const handleOpenModal = () => {
        const tableData = chartData.map((row) => {
            ...
            if (
                ...
                const categoryData =
                    row.commodities_categories[categoryName] || {}; // Handle missing categoryData gracefully
