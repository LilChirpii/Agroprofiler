    ...
    const handleOpenModal = () => {
        const tableData = chartData.map((row) => {
            ...
            if (distributionType === "farmers") {
                // Add registered and unregistered data, and calculate the total
                Object.keys(row).forEach((key) => {
                    if (key !== "name") {
                        rowDetails[key] = row[key];
                        total += row[key]; // Add values to total
                    }
                });
            } else if (distributionType === "commodityCategories") {
                // Add commodity subcategories and calculate the total
                Object.keys(row).forEach((key) => {
                    if (key !== "name") {
                        rowDetails[key] = row[key];
                        total += row[key]; // Add values to total
            ...
            rowDetails["total"] = total; // Add total to the row
            return rowDetails;
        });
                            ...
                            <TableBody>
                                {selectedData.map((row, index) => (
                                    <TableRow key={index}>
                                        ...
                                        {/* Dynamically render the data for each row */}
                                        {Object.keys(row).map(
                                            (key) =>
                                                key !== "name" &&
                                                key !== "total" && (
