...
const Heatmap: React.FC<HeatmapProps> = ({ distributionData }) => {
    ...
    const closeModal = (): void => {
      ...
      const [dynamicDistributions, setDynamicDistributions] = useState<any>({
        allocations: ["All", "Cash Assistance", "Pesticides", "Fertilizer"],
        farmers: ["All", "Registered", "Unregistered"],
        highValue: ["All", "Fruit-Bearing", "Vegetables"],
        commodities: {},
    });
    ...
    useEffect(() => {
        // Fetch commodity categories and populate the distributions
        const fetchDistributions = async () => {
            try {
                const response = await axios.get("/api/dashboard-data");
                const { commodityCategories } = response.data;

                const commodityDistributions: Record<string, string[]> = {};
                commodityCategories.forEach((category: any) => {
                    commodityDistributions[category.name] = category.commodities.map(
                        (commodity: any) => commodity.name
                    );
                });

                setDynamicDistributions((prev: any) => ({
                    ...prev,
                    commodities: commodityDistributions,
                }));
            } catch (error) {
    ...
    const getOptionsForView = () => {
        // Fixed options
        const fixedOptions = [
            { value: "allocations", label: "Allocations" },
            { value: "farmers", label: "Farmers" },
            { value: "highValue", label: "High-Value Crops" },
        ];

        // Add commodity categories dynamically
        const dynamicOptions = commodityCategories.map((category) => ({
            value: `commodityCategory:${category.name}`,
            label: category.name,
        }));
    ...
    const getOptionsForSubtype = () => {
        if (view.startsWith("commodityCategory:")) {
            const categoryName = view.split(":")[1];
            const selectedCategory = commodityCategories.find(
                (category) => category.name === categoryName
            );

            return (
                selectedCategory?.commodities.map((commodity: any) => ({
        ...
        <div className="heatmap-container">
            <div className="select-container mb-10 flex gap-4">
                ...
                <select value={subtype} onChange={handleChangeSubtype}>
                    {getOptionsForSubtype().map((option) => (
                        <option key={option.value} value={option.value}>
                            {option.label}
                        </option>
                    ))}
