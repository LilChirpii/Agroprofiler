        ...
        const tableData = chartData.map((row) => {
            ...
            if (
                ...
                data[row.name].commodities.forEach((category) => {
                    // Check for category match or include all if no category is selected
                    if (
                        !selectedCategory ||
                        category.commodities_category_name === selectedCategory
                    ) {
                        category.commodities.forEach((commodity) => {
                            // Increment total count and set individual commodity counts
                            rowDetails[commodity.name] =
                                (rowDetails[commodity.name] || 0) +
                                (commodity.count || 0);
