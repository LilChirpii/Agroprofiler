...
const GroupedBarChart: React.FC<GroupedBarChartProps> = ({
    ...
    const handleOpenModal = () => {
        const tableData = chartData.map((row) => {
            const rowData: any = { name: row.name };

            // Handle allocations if distributionType matches
            if (distributionType === "allocations" && row?.allocations) {
                Object.keys(row.allocations).forEach((allocation) => {
                    rowData[allocation] = row.allocations[allocation] || 0;
                });
            }

            // Handle commodity_categories if distributionType matches
            if (
                distributionType.startsWith("commodity_categories_") &&
                row?.commodities_categories
            ) {
                // Extract the category name dynamically from distributionType
                const categoryName = distributionType.replace(
                    "commodity_categories_",
                    ""
                );
                ...
                // Add subcategories (commodities) to the table columns
                Object.keys(categoryData).forEach((subcategory) => {
                    rowData[subcategory] = categoryData[subcategory] || 0;
                });
            }

            // Handle farmers data if distributionType matches
            if (distributionType === "farmers" && row?.farmers) {
                Object.keys(row.farmers).forEach((farmerType) => {
                    rowData[farmerType] = row.farmers[farmerType] || 0;
                });
            }
    ...
    return (
        <div>
            ...
            <PrimaryButton
                className="absolute index-20 flex top-[124.5rem] right-[10rem] p-4"
