...
class DashboardController extends Controller
{
    ...
    public function commodityCounts(){
      $commodityData = CommodityCategory::with('commodities')->get()->mapWithKeys(function ($category) {
        return [
            $category->name => $category->commodities->mapWithKeys(function ($commodity) {
                return [
                    $commodity->name => $commodity->count, // Assuming 'count' is a field in your commodity model
                ];
            })
        ];
    });

    return response()->json($commodityData);
    }
