...
class DashboardController extends Controller
{
    ...
    public function commodityCounts(){
       $commodityCounts = Commodity::with('category')
        ->select('commodity_category_id', \DB::raw('count(*) as count'))
        ->groupBy('commodity_category_id')
        ->get();

    // Prepare the data in the desired format (category name => count)
    $result = $commodityCounts->mapWithKeys(function ($commodityCount) {
        $categoryName = $commodityCount->category->name;
        return [$categoryName => $commodityCount->count];
    });
