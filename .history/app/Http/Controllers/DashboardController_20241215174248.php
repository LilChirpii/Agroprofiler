...
class DashboardController extends Controller
{
    ...
    {
        // Fetch all categories and their commodities with counts
        $categories = CommodityCategory::with(['commodities' => function ($query) {
            $query->select('id', 'name', 'commodity_category_id')
                  ->selectRaw('COUNT(*) as commodity_total')
                  ->groupBy('id', 'name', 'commodity_category_id');
        }])
        ->withCount('commodities') // Get total commodities count for each category
        ->get();
        ...
        $data = $categories->map(function ($category) {
            return [
                ...
                'commodity_category_total' => $category->commodities_count,
                'commodities' => $category->commodities->map(function ($commodity) {
                    return [
                        'commodity_name' => $commodity->name,
                        'commodity_total' => $commodity->commodity_total, // Pre-calculated total
                    ];
                })
            ];
        });
    ...
    public function commodityCounts(){
      $commodityCounts = CommodityCategory::withCount('commodities')
        ->get()
        ->map(function ($category) {
            return [
                'category' => $category->name,
                'commodities' => $category->commodities->groupBy('name')->map(function ($commodities, $name) {
                    return [
                        'name' => $name,
    ...
    return response()->json($commodityCounts);
    }
