...
public function store(Request $request)
{
    try {
        ...
        if ($request->hasFile('proof')) {
            ...
            $file->move(public_path('proof'), $fileName); // Move the file to public/proof
            $proofPath = 'proof/' . $fileName; // Path relative to the public directory
        }
...
public function update(Request $request, $id)
{
    ...
    if ($request->hasFile('proof')) {
        $file = $request->file('proof');
        $fileName = time() . '.' . $file->getClientOriginalExtension();
        $file->move(public_path('proof'), $fileName); // Store the file in public/proof
        $request->merge(['proof' => 'proof/' . $fileName]); // Update the request with the file path
    }
