...
class DashboardController extends Controller
{
    ...
    public function getCounts(Request $request){
       { $year = $request->input('year'); $allocationCounts = AllocationType::withCount(['allocations' => function ($query) use ($year) { if ($year) { $query->whereYear('created_at', $year); } }])->get(); $response = []; foreach ($allocationCounts as $type) { $response[$type->name] = $type->allocations_count; } return response()->json(
    }
